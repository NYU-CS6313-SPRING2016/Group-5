...
@app.route('/realtime_sector', methods=['GET'])
@crossdomain(origin='*')
def handle_realtime_sector_request():
    sector_name = request.args['sector']
    global realtime_analyzer
    sector = Sector(sector_name)
    if sector_name in realtime_analyzer.sectors:
        sector = realtime_analyzer.sectors[sector_name]
    return json.dumps({
        'users': len(sector.users),
        'messages': sector.num_messages,
        'positive': sector.positive,
...
@crossdomain(origin='*')
def handle_realtime_stock_request():
    symbol = request.args['symbol']
    global realtime_analyzer
    stock = Stock(symbol)
    if symbol in realtime_analyzer.stocks:
        stock = realtime_analyzer.stocks[symbol]
    return json.dumps({
        'users': len(stock.users),
        'messages': stock.num_messages,
        'positive': stock.positive,
...
@app.route('/realtime_user_classification', methods=['GET'])
@crossdomain(origin='*')
def handle_realtime_user_classification_request():
    classification_name = request.args['classification']
    global realtime_analyzer
    classification = UserClassification(classification_name)
    if classification_name in realtime_analyzer.user_classification:
        classification = realtime_analyzer.user_classification[classification_name]
    return json.dumps({
        'users': len(classification.users),
        'messages': classification.num_messages
    })
